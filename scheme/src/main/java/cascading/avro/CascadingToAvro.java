...
class CascadingToAvro {
      ...
      case RECORD:
        if (obj instanceof Record) return obj;
          ...
          if (obj instanceof Tuple) {
            objectArray = parseTuple((Tuple) obj, schema);
          } else {
            objectArray = parseTupleEntry((TupleEntry) obj, schema);
          }
  ...
  private static Object toAvroFixed(Object obj, Schema schema) {
    if (obj instanceof Fixed) {
      ...
      return new Fixed(schema, bytes.getBytes());
    }
  }
  ...
  private static Schema generateAvroSchemaFromIterable(Iterable element, String name) {
    @SuppressWarnings("unchecked") Iterator<Object> iterator = element.iterator();
    if (!iterator.hasNext()) {
  ...
  public static Schema generateAvroSchemaFromFieldsAndTypes(String recordName, Fields schemeFields,
    ...
    for (int i = 0; i < schemeTypes.length; i++) {
      if ((schemeTypes[i] == List.class) || (schemeTypes[i] == Map.class)) {
        ++i;
        if (isNotPrimitiveType(schemeTypes[i])) {
